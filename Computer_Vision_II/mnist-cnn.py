...
def model_train(args, model, device, train_loader, optimizer, epoch):
    model.train()
        ...
        for data, target in test_loader:
            ...
            if visual_ON:
                for i in range(data.shape[0]):
                    ...
                    print('prediction', i, ':', output[i].argmax())
...
def main():
    ...
    dataset_test = datasets.MNIST('data', train=False,
                       transform=transform)
    train_loader = torch.utils.data.DataLoader(dataset_training,**kwargs)
